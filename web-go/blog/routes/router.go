...
func SetupRoutes(router *gin.Engine, cfg *config.Config) {
	...
	apiPrivate := apiPublic.Use(middleware.JWTUserMiddleware(cfg))
	{
		...
		// 评论管理
		apiPrivate.POST("/posts/:id/comments", commentController.CreateComment)
		apiPrivate.DELETE("/comments/:id", middleware.CommentOwnershipMiddleware(), commentController.DeleteComment)
